"""Scraper for TJ Guyer Property Management (tjguyer.com).

Uses AppFolio listings - same structure as teamnwpm.
"""
...
class TJGuyerScraper(BaseScraper):
    """Scraper for tjguyer.com property listings via AppFolio."""

    def __init__(self, url: str = "https://tjguyer.appfolio.com/listings"):
        super().__init__(source_name="tjguyer", base_url=url)
        self.client.headers["Referer"] = "https://tjguyer.com/"

    def scrape(self) -> list[ScrapedListing]:
        """Scrape all listings from tjguyer.appfolio.com."""
        listings = []

        try:
            print(f"[tjguyer] Fetching page...")
            soup = self.fetch_page(self.base_url)

            print(f"[tjguyer] Page title: {soup.title.string if soup.title else 'No title'}")
            ...
            print(f"[tjguyer] Found {len(listing_elements)} listing elements")

            for element in listing_elements:
                listing = self._parse_listing(element)
                if listing:
                    listings.append(listing)
                    print(f"[tjguyer] Parsed: {listing.title[:40]}... - ${listing.rent or 'N/A'}")

            print(f"[tjguyer] Successfully parsed {len(listings)} listings")

        except Exception as e:
            print(f"[tjguyer] Error scraping: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _parse_listing(self, element: Tag) -> Optional[ScrapedListing]:
        ...
        try:
            ...
            if address:
                ...
                # TJ Guyer covers Thurston County area
                city_match = re.search(r'(Tumwater|Olympia|Lacey|Yelm|Rochester|Tenino|Centralia|Chehalis|Rainier|Bucoda)', address, re.I)
                if city_match:
