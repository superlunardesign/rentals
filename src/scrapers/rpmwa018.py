"""Scraper for RPM WA 018 Property Management.

Uses AppFolio listings at rpmwa018.appfolio.com.
"""
...
class RPMWA018Scraper(BaseScraper):
    """Scraper for RPM WA 018 via AppFolio."""

    def __init__(self, url: str = "https://rpmwa018.appfolio.com/listings"):
        super().__init__(source_name="rpmwa018", base_url=url)
        self.client.headers["Referer"] = "https://rpmwa018.appfolio.com/"

    def scrape(self) -> list[ScrapedListing]:
        """Scrape all listings from rpmwa018.appfolio.com."""
        listings = []

        try:
            print(f"[rpmwa018] Fetching page...")
            soup = self.fetch_page(self.base_url)

            print(f"[rpmwa018] Page title: {soup.title.string if soup.title else 'No title'}")
            ...
            print(f"[rpmwa018] Found {len(listing_elements)} listing elements")

            for element in listing_elements:
                listing = self._parse_listing(element)
                if listing:
                    listings.append(listing)
                    print(f"[rpmwa018] Parsed: {listing.title[:40]}... - ${listing.rent or 'N/A'}")

            print(f"[rpmwa018] Successfully parsed {len(listings)} listings")

        except Exception as e:
            print(f"[rpmwa018] Error scraping: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _parse_listing(self, element: Tag) -> Optional[ScrapedListing]:
        ...
        try:
            ...
            print(f"[rpmwa018] Error parsing listing: {e}")
            return None
