"""Scraper for Welcome Home Rentals.

Uses AppFolio listings at welcomehomerentals.appfolio.com.
"""
...
class WelcomeHomeRentalsScraper(BaseScraper):
    """Scraper for Welcome Home Rentals via AppFolio."""

    def __init__(self, url: str = "https://welcomehomerentals.appfolio.com/listings"):
        super().__init__(source_name="welcomehomerentals", base_url=url)
        self.client.headers["Referer"] = "https://welcomehomerentals.appfolio.com/"

    def scrape(self) -> list[ScrapedListing]:
        """Scrape all listings from welcomehomerentals.appfolio.com."""
        listings = []

        try:
            print(f"[welcomehomerentals] Fetching page...")
            soup = self.fetch_page(self.base_url)

            print(f"[welcomehomerentals] Page title: {soup.title.string if soup.title else 'No title'}")

            listing_elements = soup.select("[id^='listing_']")
            if not listing_elements:
                listing_elements = soup.select(".listing-item")

            print(f"[welcomehomerentals] Found {len(listing_elements)} listing elements")

            for element in listing_elements:
                listing = self._parse_listing(element)
                if listing:
                    listings.append(listing)
                    print(f"[welcomehomerentals] Parsed: {listing.title[:40]}... - ${listing.rent or 'N/A'}")

            print(f"[welcomehomerentals] Successfully parsed {len(listings)} listings")

        except Exception as e:
            print(f"[welcomehomerentals] Error scraping: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _parse_listing(self, element: Tag) -> Optional[ScrapedListing]:
        ...
        try:
            ...
            print(f"[welcomehomerentals] Error parsing listing: {e}")
            return None
