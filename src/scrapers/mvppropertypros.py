"""Scraper for MVP Property Pros.

Uses AppFolio listings at mvppropertypros.appfolio.com.
"""
...
class MVPPropertyProsScraper(BaseScraper):
    """Scraper for MVP Property Pros via AppFolio."""

    def __init__(self, url: str = "https://mvppropertypros.appfolio.com/listings/"):
        super().__init__(source_name="mvppropertypros", base_url=url)
        self.client.headers["Referer"] = "https://mvppropertypros.appfolio.com/"

    def scrape(self) -> list[ScrapedListing]:
        """Scrape all listings from mvppropertypros.appfolio.com."""
        listings = []

        try:
            print(f"[mvppropertypros] Fetching page...")
            soup = self.fetch_page(self.base_url)

            print(f"[mvppropertypros] Page title: {soup.title.string if soup.title else 'No title'}")

            listing_elements = soup.select("[id^='listing_']")
            if not listing_elements:
                listing_elements = soup.select(".listing-item")

            print(f"[mvppropertypros] Found {len(listing_elements)} listing elements")

            for element in listing_elements:
                listing = self._parse_listing(element)
                if listing:
                    listings.append(listing)
                    print(f"[mvppropertypros] Parsed: {listing.title[:40]}... - ${listing.rent or 'N/A'}")

            print(f"[mvppropertypros] Successfully parsed {len(listings)} listings")

        except Exception as e:
            print(f"[mvppropertypros] Error scraping: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _parse_listing(self, element: Tag) -> Optional[ScrapedListing]:
        ...
        try:
            ...
            print(f"[mvppropertypros] Error parsing listing: {e}")
            return None
