"""Scraper for Windermere Olympia Property Management.

Uses AppFolio listings at windermereolympiapm.appfolio.com.
"""
...
class WindermereScraper(BaseScraper):
    """Scraper for Windermere Olympia PM via AppFolio."""

    def __init__(self, url: str = "https://windermereolympiapm.appfolio.com/listings"):
        super().__init__(source_name="windermere", base_url=url)
        self.client.headers["Referer"] = "https://windermereolympiapm.com/"

    def scrape(self) -> list[ScrapedListing]:
        """Scrape all listings from windermereolympiapm.appfolio.com."""
        listings = []

        try:
            print(f"[windermere] Fetching page...")
            soup = self.fetch_page(self.base_url)

            print(f"[windermere] Page title: {soup.title.string if soup.title else 'No title'}")
            ...
            print(f"[windermere] Found {len(listing_elements)} listing elements")

            for element in listing_elements:
                listing = self._parse_listing(element)
                if listing:
                    listings.append(listing)
                    print(f"[windermere] Parsed: {listing.title[:40]}... - ${listing.rent or 'N/A'}")

            print(f"[windermere] Successfully parsed {len(listings)} listings")

        except Exception as e:
            print(f"[windermere] Error scraping: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _parse_listing(self, element: Tag) -> Optional[ScrapedListing]:
        ...
        try:
            ...
            print(f"[windermere] Error parsing listing: {e}")
            return None
