"""Scraper for Narrows Property Management.

Uses AppFolio listings at narrowspm.appfolio.com.
"""
...
class NarrowsPMScraper(BaseScraper):
    """Scraper for Narrows Property Management via AppFolio."""

    def __init__(self, url: str = "https://narrowspm.appfolio.com/listings"):
        super().__init__(source_name="narrowspm", base_url=url)
        self.client.headers["Referer"] = "https://narrowspm.appfolio.com/"

    def scrape(self) -> list[ScrapedListing]:
        """Scrape all listings from narrowspm.appfolio.com."""
        listings = []

        try:
            print(f"[narrowspm] Fetching page...")
            soup = self.fetch_page(self.base_url)

            print(f"[narrowspm] Page title: {soup.title.string if soup.title else 'No title'}")

            listing_elements = soup.select("[id^='listing_']")
            if not listing_elements:
                listing_elements = soup.select(".listing-item")

            print(f"[narrowspm] Found {len(listing_elements)} listing elements")

            for element in listing_elements:
                listing = self._parse_listing(element)
                if listing:
                    listings.append(listing)
                    print(f"[narrowspm] Parsed: {listing.title[:40]}... - ${listing.rent or 'N/A'}")

            print(f"[narrowspm] Successfully parsed {len(listings)} listings")

        except Exception as e:
            print(f"[narrowspm] Error scraping: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _parse_listing(self, element: Tag) -> Optional[ScrapedListing]:
        ...
        try:
            ...
            print(f"[narrowspm] Error parsing listing: {e}")
            return None
